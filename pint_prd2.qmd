...
#| echo: false
# Specify path
path <- "C:/Users/rjjanse.LUMCNET/Downloads/TBI.txt"
...
lambdas <- seq(0.001, 100, length.out = 3e4) # <4>
...
# Get optimal lambda
lambda <- cv_ridge[["lambda.min"]]         # <7>

# Show results of cross-validation          # <8>
...
4. We specify our own range of lambda's we would like to test using cross-validation. Note that `glmnet()` can also do this on our own, but this may be [unreliable](https://stackoverflow.com/questions/23686067/default-lambda-sequence-in-glmnet-for-cross-validation).
5. We specify `alpha = 0` to indicate that we want to use ridge regression. If we use `alpha = 1`, we indicate LASSO regression. Anything in between would result in elastic net regression.
6. We specify 10-fold cross-validation.
7. The optimal $\lambda$ is derived by taking `lambda.min` from the results.
8. We can show that the $\lambda$ we selected (the dashed line) is the one with the lowest mean squared error. The x-axis is in log scale for clarity.

Our optimal $\lambda$ value is then:
...
\color{red}{\alpha\sum_{j=1}^{P}\lvert\hat{\beta}_j\rvert}
\color{black}{+}
\color{blue}{\frac{1-\alpha}{2}\sum_{p=1}^P\hat{\beta}_p^2}\right)
$$
...
calmetrics <- function(fit, x = NULL, y = NULL, df = NULL){
    ...
    results <- round(fit_cal[["coefficients"]][[2]], 3)
...
calibm <- tibble(model = rep(c("standard", "ridge", "lasso", "elastic net"), 2),
                 cohort = c(rep("development", 4), rep("validation", 4)),
                 slope = c(calmetrics(fit, y = y, df = development), calmetrics(fit_ridge, x, y), 
                           calmetrics(fit_lasso, x, y), calmetrics(fit_elnet, x, y),
                           calmetrics(fit, y = y_val, df = validation), calmetrics(fit_ridge, x_val, y_val), 
                           calmetrics(fit_lasso, x_val, y_val), calmetrics(fit_elnet, x_val, y_val)))

# Pivot data
calibm %<>% pivot_wider(names_from = cohort, values_from = slope)
...
What we see is that (although small for the C-statistics), the standard model performs better in the development data, but worse in the validation data. Because the improvement is only minimal, the calibration plots do not show much difference, but it is always good practice to show the calibration plots. Note however that this does not have to be the case: [van Calster *et al.*](https://doi.org/10.1177/0962280220921415).
...
lps <- x %*% as.matrix(coefs)

# Re-estimate intercept
intercept <- glm(development[["d.unfav"]] ~ offset(lps), family = binomial)[["coefficients"]][[1]]

# Combine intercept and coefficients
bu <- c(intercept, coefs)
...
tibble(model = c("standard", "BU"),
       ...
       slope_dev = c(calmetrics(fit, y = y, df = development), calmetrics(fit_bu, y = y, df = development)),
       slope_val = c(calmetrics(fit, y = y_val, df = validation), calmetrics(fit_bu, y = y_val, df = validation)))
